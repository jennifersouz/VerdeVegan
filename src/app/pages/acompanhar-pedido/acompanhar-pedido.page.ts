...
export class AcompanharPedidoPage implements OnDestroy {
  ...
  async carregarPedido() {
    ...
    void this.sincronizarEstadoAutomatico();
    this.carregando = false;
  }
  ...
  async cancelarPedido() {
    ...
    await this.supabaseService.atualizarEstadoPedido(this.pedido.id, 'Cancelado');
    await this.ajustarPontosPorCancelamento();
  }

  async confirmarEntregaEAvaliacao() {
    if (!this.pedido) {
      return;
    }
    ...
    this.pedidosService.atualizarPedido(this.pedido);
    await this.supabaseService.atualizarEstadoPedido(this.pedido.id, 'Entregue');
    await this.atribuirPontosGanhosPorEntrega();
  }
  ...
  private async sincronizarEstadoAutomatico() {
  ...
  private async ajustarPontosPorCancelamento() {
    const pontosUsados = this.pedido?.pontosUsados || 0;
  ...
  private async atribuirPontosGanhosPorEntrega() {
    const pontosGanhos = this.pedido?.pontosGanhos || 0;
