...
export class CarrinhoPage implements OnDestroy {
  ...
  async confirmarPedido() {
    ...
    const tempoEntregaMinutos = await this.calcularTempoEntregaMinutos();
    const pedido: Pedido = {
    ...
    await this.atualizarPontosDepoisPedido(pedido);
    this.itens = [];
  ...
  private async calcularTempoEntregaMinutos(): Promise<number> {
    const pratos = await this.carregarPratosMenu();
    const temposPorRestaurante = new Map<string, number[]>();

    this.itens.forEach((item: ItemCarrinho) => {
      const prato = pratos.find((pratoMenu: Prato) =>
        pratoMenu.nome.toLowerCase() === item.nome.toLowerCase()
      );
      const restaurante = item.restaurante || prato?.restaurante || 'VerdeVegan';
      const tempoTexto = item.tempo || prato?.tempo;
      const tempo = this.extrairMediaMinutos(tempoTexto);

      if (!tempo) {
        return;
      }

      const tempos = temposPorRestaurante.get(restaurante) || [];
      tempos.push(tempo);
      temposPorRestaurante.set(restaurante, tempos);
    });

    const mediasRestaurantes = Array.from(temposPorRestaurante.values())
      .map((tempos) => tempos.reduce((total, tempo) => total + tempo, 0) / tempos.length);

    if (mediasRestaurantes.length === 0) {
      return 6;
    }

    const media = mediasRestaurantes.reduce((total, tempo) => total + tempo, 0) / mediasRestaurantes.length;
    return Math.max(6, Math.round(media));
  }

  private carregarPratosMenu(): Promise<Prato[]> {
    return new Promise((resolve) => {
      this.menuService.carregarPratos().subscribe({
        next: (pratos) => resolve(pratos),
        error: () => resolve([])
      });
    });
  }

  private extrairMediaMinutos(tempo?: string): number {
    if (!tempo) {
      return 0;
    }

    const valores = tempo.match(/\d+/g)?.map((valor) => Number(valor)) || [];
  ...
  private async atualizarPontosDepoisPedido(pedido: Pedido) {
    if (!this.perfilAtual) {
      return;
    }

    if (this.pontosARedimir > 0) {
    ...
    const perfilAtualizado = await this.perfilService.obterPerfil();
