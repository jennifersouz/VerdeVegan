...
export class PedidosPage implements OnInit, OnDestroy {
  ...
  async carregarPedidos() {
    ...
    this.pedidos = await Promise.all(pedidos.map((pedido) => this.sincronizarEstadoPedido(pedido)));
  }
  ...
  private async sincronizarEstadoPedido(pedido: Pedido): Promise<Pedido> {
    ...
    await this.supabaseService.atualizarEstadoPedido(pedidoAtualizado.id, estado);

    if (estado === 'Entregue' && pedidoAtualizado.pontosGanhos) {
      await this.perfilService.adicionarPontosPedido(
        'Acumulaste',
        pedidoAtualizado.pontosGanhos,
        pedidoAtualizado.id,
        'pontos_entrega'
      );
    }
