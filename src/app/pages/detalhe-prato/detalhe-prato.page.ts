...
export class DetalhePratoPage implements OnInit {
  ...
  // ── Ação principal do footer ──────────────────────────────────────────────
  /**
   * Pratos personalizáveis → vão para a página de personalização.
   * Pratos NÃO personalizáveis → são adicionados diretamente ao carrinho
   * (funciona em modo convidado graças ao CarrinhoService).
   */
  public async acaoPrincipal() {
    if (!this.prato) return;

    if (this.prato.personalizavel) {
      this.personalizar();
    } else {
      await this.adicionarDiretoAoCarrinho();
    }
  }

  public personalizar() {
    if (!this.prato?.personalizavel) return;
    ...
    this.router.navigateByUrl(
      `/tabs/personalizar-prato/${this.prato.id}?qtd=${this.quantidade}${parametrosOrigem}`
    );
  }

  public async adicionarDiretoAoCarrinho() {
    if (!this.prato) return;
    ...
    const item = {
      ...
      selecoes: {},
      observacoes: '',
      totalUnidade: this.prato.preco,
      totalFinal: this.prato.preco * this.quantidade
    };

    await this.carrinhoService.adicionarItem(item);
  ...
  public obterImagemRestaurante(): string {
    ...
    return this.restaurantes
      .find(r => r.nome === this.prato?.restaurante)?.imagem || '';
  }

  public abrirRestaurante() {
    const restaurante = this.restaurantes.find(r => r.nome === this.prato?.restaurante);
    if (!this.prato || !restaurante) return;
    ...
    if (this.restauranteId)  params.set('detalheRestauranteId', String(this.restauranteId));
    if (this.pesquisaMenu)   params.set('pesquisa', this.pesquisaMenu);
    if (this.tipoMenu)       params.set('tipo', this.tipoMenu);
    if (this.categoriaMenu)  params.set('categoria', this.categoriaMenu);
