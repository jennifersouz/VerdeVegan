...
export class PerfilService {
  ...
  public async obterEmailUtilizadorAtual(): Promise<string | null> {
    ...
    const emailSessao = await this.supabaseService.obterEmailUtilizadorAtual();

    if (emailSessao) {
      await this.storage.set(this.CHAVE_EMAIL_ATUAL, emailSessao);
      return emailSessao;
    }
  ...
  public async obterPerfil(): Promise<UtilizadorPerfil | null> {
    ...
    const utilizadores = await this.obterTodosUtilizadores();
    const perfilLocal = utilizadores[emailAtual];

    if (perfilLocal) {
      await this.supabaseService.guardarPerfil(perfilLocal);
      return perfilLocal;
    }

    const perfilSessao: UtilizadorPerfil = {
      nome: emailAtual.split('@')[0],
      email: emailAtual,
      telefone: '',
      morada: '',
      metodoPagamento: '',
    ...
    await this.guardarPerfilLocal(perfilSessao);
    await this.supabaseService.guardarPerfil(perfilSessao);
  ...
  public async adicionarPontos(descricao: string, pontos: number): Promise<void> {
    ...
    const novoItem: HistoricoPontos = {
      ...
      ordenadoEm: new Date().toISOString()
    };
  ...
  public async adicionarPontosPedido(
    descricao: string,
    pontos: number,
    pedidoId: string,
    tipoMovimento: string
    ...
    const pedidoNormalizado = pedidoId.trim().replace(/^#/, '');
    const historicoGuardado: HistoricoPontos[] =
      (await this.storage.get(this.CHAVE_HISTORICO_PONTOS)) || [];

    const movimentoJaExiste = historicoGuardado.some((item: HistoricoPontos) =>
      item.email === emailAtual &&
      item.pedidoId === pedidoNormalizado &&
      item.tipoMovimento === tipoMovimento
    );
  ...
  private ordenarHistoricoPontos(historico: HistoricoPontos[]): HistoricoPontos[] {
    return [...historico].sort((a: HistoricoPontos, b: HistoricoPontos) => {
      const dataA = this.obterTimestampHistorico(a);
      const dataB = this.obterTimestampHistorico(b);
  ...
  private obterTimestampHistorico(item: HistoricoPontos): number {
    const valor = item.ordenadoEm || item.data;
